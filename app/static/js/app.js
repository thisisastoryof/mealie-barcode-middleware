...
(function() {
        ...
        fetch('/api/barcodes?status=' + encodeURIComponent(status)).then(function(r) { return r.json(); }).then(function(d) {
            ...
            d.items.forEach(function(item) {
                rows += '<tr>'
                    ...
                    + '<td>';
                if (item.food_name) {
                    rows += '<a href="/foods/' + esc(item.food_id) + '">' + esc(item.food_name) + '</a>'
    ...
    function statusDotClass(result) {
        if (result === 'retry_failed') return 'bg-red';
    ...
    function priorityIcon(result) {
        if (result === 'retry_failed') return '<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-sm text-danger" width="16" height="16" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 9v4"/><path d="M10.363 3.591l-8.106 13.534a1.914 1.914 0 0 0 1.636 2.871h16.214a1.914 1.914 0 0 0 1.636 -2.87l-8.106 -13.536a1.914 1.914 0 0 0 -3.274 0z"/><path d="M12 16h.01"/></svg>';
